...
public interface TotpResetRequestRepository extends JpaRepository<TotpResetRequest, UUID> {
    ...
    @Query(value = 
           "SELECT COALESCE(AVG(TIMESTAMPDIFF(HOUR, request_time, processed_time)), 0) " +
           "FROM totp_reset_requests " +
           "WHERE processed = true AND processed_time IS NOT NULL", 
           nativeQuery = true)
    double findAverageProcessingTimeInHours();
    
    // Method to count requests per day for a date range
    @Query(value = 
           "SELECT DATE_FORMAT(request_time, '%Y-%m-%d') AS date, COUNT(*) AS count " +
           "FROM totp_reset_requests " +
