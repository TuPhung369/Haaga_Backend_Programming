...
public class RecaptchaServiceImpl implements RecaptchaService {
  ...
  public RecaptchaServiceImpl() {
    ...
    log.info("RecaptchaServiceImpl initialized with active profile: {}", activeProfile);
    log.info("Development mode forced: {}", FORCE_DEV_MODE);
  }

  @Override
  public void validateRecaptchaToken(String token) {
    if (!isValidToken(token)) {
      throw new AppException(ErrorCode.RECAPTCHA_VALIDATION_FAILED, "Invalid reCAPTCHA token");
    }
  }

  @Override
  public boolean isValidToken(String token) {
    // Validate using V3 token logic
   ...
   */
  private boolean validateToken(String token, boolean isV2) {
  ...
  public boolean validateHybrid(String v3Token, String v2Token) {
    ...
    try {
      if (validateToken(v3Token, false)) {
