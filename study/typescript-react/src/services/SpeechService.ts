...
/**
 * Specialized function for Finnish speech-to-text conversion
 * Uses a smaller 'tiny' Whisper model for faster processing with reduced timeout
 * @param audioBlob - The audio blob to transcribe
 * @returns The transcribed text
 */
async function convertFinnishSpeechToText(
  audioBlob: Blob
  ...
  // Create FormData for the request - SIMPLIFIED VERSION
  ...
  console.log(`🎤 FINNISH: FormData entries:`);
  // Define a more specific type for FormData that includes the entries method
  interface FormDataWithEntries extends FormData {
    entries(): IterableIterator<[string, FormDataEntryValue]>;
  }

  // Cast to our specific interface instead of using 'any'
  const formDataWithEntries = formData as FormDataWithEntries;
  if (typeof formDataWithEntries.entries === "function") {
    for (const pair of formDataWithEntries.entries()) {
      if (pair[0] === "file" && pair[1] instanceof File) {
        console.log(
          `🎤 FINNISH: ${pair[0]}: ${pair[1].name}, size: ${pair[1].size} bytes, type: ${pair[1].type}`
        );
      } else {
        console.log(`🎤 FINNISH: ${pair[0]}: ${pair[1]}`);
      }
    }
  } else {
    console.log(
      `🎤 FINNISH: FormData logging not supported in this environment`
    );
  }
  ...
  try {
    console.log(
      `🎤 FINNISH: Performing health check before Finnish processing`
    );
    ...
    if (!healthResponse.ok) {
      console.warn(
        `🎤 FINNISH: Server health check failed with status: ${healthResponse.status}`
      );
      return {
        transcript:
    ...
    console.error(`🎤 FINNISH: Server health check failed:`, healthError);
    return {
  ...
  console.log(`🎤 FINNISH: Using endpoint: ${serverUrl}`);

  const controller = new AbortController();
  const timeoutId = setTimeout(() => {
    controller.abort();
    console.error(
      `🎤 FINNISH: Request timed out after ${timeoutValue / 1000} seconds`
    );
  }, timeoutValue);

  try {
    console.log(`🎤 FINNISH: Sending fetch request to server`);
    ...
    console.log(
      `🎤 FINNISH: Received response with status: ${response.status}`
    );
    ...
    if (response.ok) {
      try {
        ...
        console.log(
          `🎤 FINNISH: Speech to text completed in ${processingTime}ms: "${result.transcript}"`
        );
        ...
        if (processingTime < 10000) {
          ...
          console.log(
            `🎤 FINNISH: Fast processing achieved using tiny Whisper model`
          );
        }
        return result;
      } catch (parseError) {
        console.error(`🎤 FINNISH: Error parsing response: ${parseError}`);
        return {
      ...
      console.error(`🎤 FINNISH: Server error: ${errorBody}`);
    ...
    console.error(`🎤 FINNISH: Fetch request error: ${errorMessage}`);
    return {
      transcript: `Error: ${
...
/**
 * Converts speech from audio blob to text using appropriate processing for the language
 * @param audioBlob - The audio blob to transcribe
 * @param language - The language of the audio (e.g., 'en-US' for English, 'fi-FI' for Finnish)
 * @returns The transcribed text
 */
export async function convertSpeechToText(
  ...
  // Check if this is Finnish and use specialized processing if needed
  const isFinnish = language.toLowerCase().includes("fi");
  if (isFinnish) {
    return convertFinnishSpeechToText(audioBlob);
  }
